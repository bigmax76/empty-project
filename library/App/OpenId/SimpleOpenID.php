...
class App_OpenId_SimpleOpenID {
        ...
        function SetOpenIDUser($s){
            $this->openid_user = $s;
        }
        
        function GetOpenIDUser(){
            //var_dump($_GET);
            if (!empty($_GET['openid_ext1_value_firstname']) && !empty($_GET['openid_ext1_type_lastname'])){
                return urldecode($_GET['openid_ext1_value_firstname'].' '.$_GET['openid_ext1_value_lastname']);
            }else if (!empty($_GET['openid_ext1_value_email'])){
                return urldecode($_GET['openid_ext1_value_email']);
            }else{
                $id = $this->GetIdentity();
                if (preg_match('/^http[s]*:\/\/([\S]+)$/', $id, $arr)){
                    if (strrpos($arr[1],'/') == strlen($arr[1])-1){
                        return substr($arr[1], 0, strlen($arr[1])-1);
                    }else{
                        return $arr[1];
                    }
                }else{
        ...
        function SetGoogleOpenID(){
            $this->google_openid = 1;
        }
...
//*/
        ...
        function CURL_Request($url, $method="GET", $params = "") { // Remember, SSL MUST BE SUPPORTED
         ...
         function HTML2OpenIDServer($content) {
            ...
            if (preg_match('/<xrds:/',$content)){
                $this->SetGoogleOpenID();
                preg_match_all('/<URI>([^>]*)<\/URI>/', $content, $servers);
                $ret = array($servers[1], array());
            }else{
                preg_match_all('/<link[^>]*rel=[\'"][openid2.provider ]*openid.server[\'"][^>]*href=[\'"]([^\'"]+)[\'"][^>]*\/?>/', $content, $matches1);
                preg_match_all('/<link[^>]*href=\'"([^\'"]+)[\'"][^>]*rel=[\'"]openid.server[\'"][^>]*\/?>/', $content, $matches2);
                
                $servers = array_merge($matches1[1], $matches2[1]);
                
                preg_match_all('/<link[^>]*rel=[\'"]openid.delegate[\'"][^>]*href=[\'"]([^\'"]+)[\'"][^>]*\/?>/', $content, $matches1);
                
                preg_match_all('/<link[^>]*href=[\'"]([^\'"]+)[\'"][^>]*rel=[\'"]openid.delegate[\'"][^>]*\/?>/', $content, $matches2);
        ...
        function GetOpenIDServer(){
            ...
            list($servers, $delegates) = $this->HTML2OpenIDServer($response);
                //var_dump($servers, $delegates);exit;
                if (count($servers) == 0){
        ...
        $params['openid.ax.type.email'] = 'http%3A%2F%2Faxschema.org%2Fcontact%2Femail';
        $params['openid.ax.type.language'] = 'http%3A%2F%2Faxschema.org%2Fpref%2Flanguage';
        ...
        $params['openid.ax.required'] = urlencode(implode(',',$this->fields['required']));
        ...
        function ValidateWithServer(){
                ...
                if ($openid_server == false){
                        return false;
                }elseif (preg_match('/[google.com|gmail.com|googlemail.com]/',$openid_server)){
                    return true;
                }
